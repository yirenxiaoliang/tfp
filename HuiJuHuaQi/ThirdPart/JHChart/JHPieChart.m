...
-(void)reloadChartWithDataSource:(NSArray *)newDataArray{
    ...
    for (GCChartModel *model in newDataArray) {
        
        [values addObject:@(model.value)];
        if (model.name) {
            [descs addObject:model.name];
        }
        if (model.color) {
    ...
    self.descArr = descs;
    self.colorArr = colors.count == 0?k_COLOR_STOCK:colors;
...
-(void)showAnimation{
    ...
    if (_valueArr.count>0) {
        ...
        for (JHPieItemsView *itemsView in _layersArr) {
...
//            itemsView.center = CGPointMake(self.frame.size.width/2, 10+wid/2);
            ...
            JHPieItemsView *itemsView = [[JHPieItemsView alloc] initWithFrame:CGRectMake(10, 10, SCREEN_WIDTH/3-25, SCREEN_WIDTH/3-25) andBeginAngle:[_countPreAngeleArr[i] floatValue] andEndAngle:[_countPreAngeleArr[i+1] floatValue] andFillColor:colors[i%colors.count]];
...
//        _pieForeView = [[JHPieForeBGView alloc] initWithFrame:CGRectMake(10, 10, wid,  wid)];
        ...
        if (_pieForeView == nil) {
            ...
            _pieForeView = [[JHPieForeBGView alloc] initWithFrame:CGRectMake(10, 10, (SCREEN_WIDTH/3-25) * 2,  (SCREEN_WIDTH/3-25) * 2)];
        }
