...
#define kThumbnailGroupLength 78.0f
#define kThumbnailLength    SCREEN_WIDTH/4-5
...
//    UIImage *videoIcon=[UIImage imageNamed:@"勾选中22"];
//    
...
+ (void)initialize
{
    checkedIcon     = [UIImage imageNamed:@"勾选中22"];
...
//        _selectView=[[UIImageView alloc] initWithFrame:CGRectMake(frame.size.width-checkedIcon.size.width-2, 2, checkedIcon.size.width, checkedIcon.size.height)];
        _selectView=[[UIImageView alloc] initWithFrame:CGRectMake(frame.size.width-22-5, 5, 22, 22)];
...
-(void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event{
    ...
    if ((_selected=!_selected)) {
        ...
        [_selectView setImage:[UIImage imageNamed:@"未选中22"]];
    }
...
@property(nonatomic,strong) ZYQAssetViewController *vc ;
@end
...
- (id)initWithFrame:(CGRect)frame
{
    if (self = [super initWithFrame:frame])
    {
        ...
        _tapAssetView=[[ZYQTapAssetView alloc] initWithFrame:CGRectMake(frame.size.width-50, 0, 50, 50)];
...
- (void) showImage{
    // 显示大图
    [_vc showImage:[_vc.assets indexOfObject:_asset]] ;
}
...
@interface ZYQAssetViewController ()<ZYQAssetViewCellDelegate,MWPhotoBrowserDelegate>{
    int columns;
...
- (id)init
{
    ...
    {
        self.tableView.contentInset=UIEdgeInsetsMake(2.0, 4.0, 0, 4.0);
...
- (void)willAnimateRotationToInterfaceOrientation:(UIInterfaceOrientation)toInterfaceOrientation duration:(NSTimeInterval)duration
{
    ...
    {
        self.tableView.contentInset=UIEdgeInsetsMake(2.0, 4, 0, 4);
...
- (void)setupButtons
{
    ...
    doneBtn.frame = CGRectMake(0, 0, 45, 30) ;
    [doneBtn setTitle:@"完成" forState:UIControlStateNormal] ;
    [doneBtn setTitleColor:NaviItemColor forState:UIControlStateNormal] ;
    [doneBtn setTitleColor:NaviItemColor forState:UIControlStateDisabled] ;
    doneBtn.titleLabel.font = FONT(17) ;
...
//    [btn setTitleColor:NaviItemColor forState:UIControlStateNormal] ;
    [btn setImage:[UIImage imageNamed:@"关闭"] forState:UIControlStateNormal];
    [btn setImage:[UIImage imageNamed:@"关闭"] forState:UIControlStateHighlighted];
...
- (NSString *)photoBrowser:(MWPhotoBrowser *)photoBrowser titleForPhotoAtIndex:(NSUInteger)index{
    return [NSString stringWithFormat:@"%ld/%ld",index+1,_assets.count] ;
}
...
- (BOOL)shouldSelectAsset:(ALAsset *)asset select:(BOOL)select
{
    ...
    if (_indexPathsForSelectedItems.count >= vc.maximumNumberOfSelection &&!select) {
        ...
        [MBProgressHUD showError:[NSString stringWithFormat:@"最多选择%ld张",vc.maximumNumberOfSelection ] toView:self.view];
    }
...
//    self.title = [NSString stringWithFormat:format, (long)indexPaths.count];
    ...
    titleText.text = [NSString stringWithFormat:format, (long)indexPaths.count];
}
...
- (void)bind:(ALAssetsGroup *)assetsGroup
{
    ...
    float scale                 = height / kThumbnailGroupLength;
...
- (void)setupViews
{
    self.tableView.rowHeight = kThumbnailGroupLength + 12;
...
- (void)setupButtons
{
    ...
    if (picker.showCancelButton)
    {
        ...
        [doneBtn setTitleColor:NaviItemColor forState:UIControlStateNormal] ;
        [doneBtn setTitleColor:GreenColor forState:UIControlStateDisabled] ;
        doneBtn.titleLabel.font = FONT(17) ;
...
- (void)showNotAllowed
{
    ...
    UIImageView *padlock    = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"btn_icon_comment_sel"]] ;
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    return kThumbnailGroupLength + 15;
}
