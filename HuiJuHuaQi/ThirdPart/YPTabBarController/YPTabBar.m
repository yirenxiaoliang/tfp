...
- (void)updateItemsFrame {
    ...
    if (self.scrollView) {
        ...
        self.scrollView.contentSize = CGSizeMake(MAX(x + (self.rightSpacing == 0 ? self.leftAndRightSpacing : self.rightSpacing), self.scrollView.frame.size.width),
        ...
        CGFloat x = self.leftAndRightSpacing;
        CGFloat allItemsWidth = self.frame.size.width - self.leftAndRightSpacing - (self.rightSpacing == 0 ? self.leftAndRightSpacing : self.rightSpacing);
...
-(void)setCorner:(BOOL)corner{
    _corner = corner;
    if (corner) {
        self.itemSelectedBgImageView.layer.cornerRadius = 4;
        self.itemSelectedBgImageView.layer.masksToBounds = YES;
    }
}
