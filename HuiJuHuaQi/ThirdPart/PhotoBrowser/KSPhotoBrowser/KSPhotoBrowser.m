...
@interface KSPhotoBrowser () <UIScrollViewDelegate, UIViewControllerTransitioningDelegate, CAAnimationDelegate,TFPictureDownloadViewDelegate> {
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    // 尹明亮添加
    if (self.showType == KSPhotoBrowserTypeShareDownload) {
...
//        [self setupHistoryView];
    ...
    //陈宇亮添加
    if (self.showType == KSPhotoBrowserTypeShareHistory) {
...
- (void)setupDownloadView{
    ...
    TFPictureDownloadView *view = [[TFPictureDownloadView alloc] initWithFrame:(CGRect){0,SCREEN_HEIGHT-80,SCREEN_WIDTH,80}];
    view.delegate = self;
    ...
    for (NSInteger i = 0; i < 2; i++) {
        ...
        TFDownloadModel *model = [[TFDownloadModel alloc] init];
        [arr addObject:model];
        ...
        if (i == 0) {
            ...
            model.name = @"下载";
            model.image = @"下载picture";
        }
    }
    [view pictureDownloadViewWithModels:arr];
    
    
    
    UILabel *titleLabel = [[UILabel alloc] initWithFrame:(CGRect){50,20,SCREEN_WIDTH-100,44}];
    [self.view addSubview:titleLabel];
    titleLabel.textAlignment = NSTextAlignmentCenter;
    titleLabel.font = FONT(18);
    titleLabel.textColor = WhiteColor;
    self.titleLabel = titleLabel;
    self.titleLabel.text = self.fileTitle;
    
    
    self.historyBtn.frame = CGRectMake(SCREEN_WIDTH-15-70, 21, 70, 44);
    [self.historyBtn setTitle:@"历史版本" forState:UIControlStateNormal];
    [self.historyBtn setTitleColor:HexColor(0xFFFFFF) forState:UIControlStateNormal];
    self.historyBtn.contentMode = UIViewContentModeCenter;
    self.historyBtn.titleLabel.font = FONT(16);
    [self.historyBtn addTarget:self action:@selector(historyClick:) forControlEvents:UIControlEventTouchUpInside];
    [self.view addSubview:self.historyBtn];
}

- (void)historyClick:(UIButton *)button {

    if ([self.delegate respondsToSelector:@selector(historyVersionBtn:)]) {
        [self.delegate historyVersionBtn:button];
    }
...
-(void)pictureDownloadView:(TFPictureDownloadView *)pictureDownloadView withIndex:(NSInteger)index{
    ...
    if (self.showType == KSPhotoBrowserTypeShareDownload||self.showType == KSPhotoBrowserTypeShareHistory) {
...
//                KSPhotoItem *item = [_photoItems objectAtIndex:_currentPage];
                KSPhotoView *photoView = [self photoViewForPage:_currentPage];
                [AlertView showAlertView:@"提示" msg:@"保存到手机相册" leftTitle:@"取消" rightTitle:@"确定" onLeftTouched:^{
                    
                } onRightTouched:^{
                    [self loadImageFinished:photoView.imageView.image];
                }];
            }
                break;
