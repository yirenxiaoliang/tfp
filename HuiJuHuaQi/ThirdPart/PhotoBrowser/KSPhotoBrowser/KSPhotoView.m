...
- (void)setItem:(KSPhotoItem *)item determinate:(BOOL)determinate {
    ...
    if (item && !item.messageObject) {
    ...
    }else if (item && item.messageObject){
        ...
        __weak typeof(self) wself = self;
        [((JMSGImageContent *)item.messageObject) largeImageDataWithProgress:^(float percent,NSString *msgId){
            __strong typeof(wself) sself = wself;
            sself.progressLayer.strokeEnd = MAX(percent, 0.01);
        } completionHandler:^(NSData *data,NSString *objectId, NSError *error) {
            if (error == nil) {
                HQLog(@"下载大图 success the image message id is %@",objectId);
