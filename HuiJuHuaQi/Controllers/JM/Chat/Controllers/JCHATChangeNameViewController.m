...
#define kCharNumberColor HexAColor(0xbbbbbb,1)
#define kBaseLineColor HexAColor(0x3f80de,1)
#define kNameTextFieldColor HexAColor(0x2d2d2d,1)
#define kSuggestTextColor HexAColor(0xbbbbbb,1)
...
- (void)textFieldDidChangeName {
  if (self.nameTextField.text.length > 30) {
    ...
    [MBProgressHUD showMessage:[NSString stringWithFormat:@"最多输入 %ld 个字符",st_nameLengthLime] view:self.view];
    return;
  }
  self.charNumber.text = [NSString stringWithFormat:@"%ld",st_nameLengthLime - self.nameTextField.text.length];
}
