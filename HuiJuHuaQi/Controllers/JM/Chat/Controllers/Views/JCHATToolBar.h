...
typedef enum {
    JCHATToolBarTypeGood = 1,// 语音、输入框、点赞、加号（任务详情界面）
    JCHATToolBarTypeChat,// 语音、输入框、加号（聊天界面）
    JCHATToolBarTypeCircle,// 输入框（企业圈）
    JCHATToolBarTypeUploadVioce// 语音框（上传文件中的语音）
...
/** 是否显示点赞按钮 */
@property (nonatomic, assign) BOOL heartShow;
@property (nonatomic, assign) BOOL recorderType;

/** toolBar类型 */
@property (nonatomic, assign) JCHATToolBarType toolBarType;


- (void)switchToVoiceInputMode ;
@property (weak, nonatomic) IBOutlet NSLayoutConstraint *textViewLeftW;

@property (weak, nonatomic) IBOutlet NSLayoutConstraint *textViewRightW;
