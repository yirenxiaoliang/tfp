...
@interface JCHATPersonViewController () <HQChatTableViewDelegate,
UIAlertViewDelegate,
UIGestureRecognizerDelegate,
UIPickerViewDataSource,
UIPickerViewDelegate> {
  
  HQChatTableView *_personTabl;
  NSArray *_titleArr;
  NSArray *_imgArr;
...
- (void)viewDidLoad {
  ...
  _personTabl = [[HQChatTableView alloc] initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT - NavigationBarHeight - kStatusBarHeight)];
  ...
  _genderPicker = [[UIPickerView alloc]initWithFrame:CGRectMake(0, SCREEN_HEIGHT, SCREEN_WIDTH, 100)];
...
- (void)tableView:(UITableView *)tableView
  ...
  if (_selectFlagGender) {
    ...
    [JMSGUser updateMyInfoWithParameter:_genderNumber userFieldType:kJMSGUserFieldsGender completionHandler:^(id resultObject, NSError *error) {
      if (error == nil) {
        HQLog(@"Action updateMyInfoWithPareter success");
      } else {
        HQLog(@"Action updateMyInfoWithPareter fail");
      }
    }];
    _selectFlagGender = NO;
  }
