...
/** JCHATFootTableCollectionReusableView *footTable */
@property (nonatomic, strong) JCHATFootTableCollectionReusableView *footTable;
...
- (void)viewDidLoad {
  ...
  if (_conversation.conversationType == kJMSGConversationTypeSingle) {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(asynchronizeFinish) name:AsynchronizeFinishNotification object:nil];
}

- (void)asynchronizeFinish{
    if (_conversation.conversationType == kJMSGConversationTypeSingle) {
        _isNoDisturb = ((JMSGUser *)_conversation.target).isNoDisturb;
    }else {
        _isNoDisturb = ((JMSGGroup *)_conversation.target).isNoDisturb;
    }
    [self.footTable.footTableView reloadData];
}
...
- (CGSize)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout *)collectionViewLayout referenceSizeForFooterInSection:(NSInteger)section {
    if (_conversation.conversationType == kJMSGConversationTypeSingle) {
        ...
        return CGSizeMake(SCREEN_WIDTH, 2 * 66);
    } else{
...
- (UICollectionReusableView *)collectionView:(UICollectionView *)collectionView viewForSupplementaryElementOfKind:(NSString *)kind atIndexPath:(NSIndexPath *)indexPath {
    ...
    if (!self.footTable) {
        ...
        footTable = [_groupMemberGrip dequeueReusableSupplementaryViewOfKind:UICollectionElementKindSectionFooter withReuseIdentifier:footerIdentifier forIndexPath:indexPath];
        footTable.footTableView.delegate = self;
        footTable.footTableView.dataSource =self;
        [self.footTable.footTableView reloadData];
        self.footTable = footTable;
    }
    return self.footTable;
}
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex
{
  ...
  switch (alertView.tag) {
        ...
        [((JMSGGroup *)(self.conversation.target)) addMembersWithUsernameArray:@[[alertView textFieldAtIndex:0].text] appKey:JMESSAGE_APPKEY
                                                             completionHandler:^(id resultObject, NSError *error) {
                                                               ...
                                                               if (error == nil) {
                                                                 ...
                                                                 HQLog(@"addMembersFromUsernameArray fail with error %@",error);
    ...
    case kAlertViewTagQuitGroup:
    {
      if (buttonIndex ==1) {
        __weak __typeof(self)weakSelf = self;
        [MBProgressHUD showMessage:@"正在退出群组！" toView:self.view];
        ...
        [deletedGroup exit:^(id resultObject, NSError *error) {
          ...
          if (error == nil) {
            HQLog(@"退出群组成功");
            [MBProgressHUD showMessage:@"退出群组成功" view:weakSelf.view];
            ...
            HQLog(@"退出群组失败");
            [MBProgressHUD showMessage:@"退出群组失败" view:weakSelf.view];
          }
        }];
      }
    }
...
//            [strongSelf.sendMessageCtl setupView];
            ...
            HQLog(@"creategroupconversation error with error : %@",error);
          }
        }];
      } else {
  ...
  [((JMSGGroup *)(self.conversation.target)) removeMembersWithUsernameArray:@[userName] appKey:JMESSAGE_APPKEY
                                                          completionHandler:^(id resultObject, NSError *error) {
                                                            ...
                                                            if (error == nil) {
                                                              ...
                                                              HQLog(@"JCHATGroupSettingCtl   fail to removeMembersFromUsernameArrary");
                                                              [MBProgressHUD hideAllHUDsForView:self.view animated:YES];
