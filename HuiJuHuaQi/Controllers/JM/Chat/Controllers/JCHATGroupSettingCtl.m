...
- (void)viewDidLoad {
  ...
  HQLog(@"Action - viewDidLoad");
  _groupTitleData =@[@"群聊名称",@"清空聊天记录",@"删除并退出"];
  
  self.groupTab = [[HQChatTableView alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH,self.view.frame.size.height - 64)];
  ...
  _headView = [[UIScrollView alloc]initWithFrame:CGRectMake(10, 0, SCREEN_WIDTH, kheadViewHeight)];
  self.groupTab.tableHeaderView = _headView;
  [_headView setBackgroundColor:[UIColor clearColor]];
  _headLine = [[UIView alloc]initWithFrame:CGRectMake(0, kheadViewHeight-1, SCREEN_WIDTH, 1)];
      ...
      if (i*4 +j == [_groupData count]) {
        ...
        [user thumbAvatarData:^(NSData *data, NSString *objectId, NSError *error) {
          if (error == nil) {
            ...
            HQLog(@"JCHATDetailsInfoVC thumbAvatarData fail");
            [personView.headViewBtn setImage:[UIImage imageNamed:@"headDefalt"] forState:UIControlStateNormal];
          }
        }];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
  if (indexPath.row !=[_groupTitleData count]-1) {
    ...
    if (cell == nil) {
      ...
      UIView *cellLine = [[UIView alloc]initWithFrame:CGRectMake(0, 53, SCREEN_WIDTH, 1)];
