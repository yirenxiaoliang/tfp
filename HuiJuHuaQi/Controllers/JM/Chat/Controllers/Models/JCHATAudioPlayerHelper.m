...
@implementation JCHATAudioPlayerHelper
...
- (void)playAudioWithURL:(NSString *)url {
    if (url != nil) {
        ...
        NSURL *someURL = [NSURL URLWithString:url];
        
        NSData *audioData = [NSData dataWithContentsOfURL:someURL];
        
        AVAudioPlayer *pl = [[AVAudioPlayer alloc] initWithData:audioData error:&error];
...
+ (id)shareInstance {
    static JCHATAudioPlayerHelper *instance = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        instance = [[JCHATAudioPlayerHelper alloc] init];
    });
    return instance;
}
...
- (void)sensorStateChange:(NSNotificationCenter *)notification {
    ...
    if ([[UIDevice currentDevice] proximityState] == YES) {
        //黑屏
        HQLog(@"Device is close to user");
        [[AVAudioSession sharedInstance] setCategory:AVAudioSessionCategoryPlayAndRecord error:nil];
        
    } else {
        //没黑屏幕
        HQLog(@"Device is not close to user");
        [[AVAudioSession sharedInstance] setCategory:AVAudioSessionCategoryPlayback error:nil];
        if (!_player || !_player.isPlaying) {
