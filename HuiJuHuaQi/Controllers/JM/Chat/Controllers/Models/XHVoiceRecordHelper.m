...
- (void)startRecordingWithPath:(NSString *)path StartRecorderCompletion:(XHStartRecorderCompletion)startRecorderCompletion {
    ...
    if(error) {
        HQLog(@"audioSession: %@ %ld %@", [error domain], (long)[error code], [[error userInfo] description]);
        return;
    }
...
- (void)getVoiceDuration:(NSString*)recordPath {
    ...
    HQLog(@"时长:%f", play.duration);
    self.recordDuration = [NSString stringWithFormat:@"%.1f", play.duration+1.0];
