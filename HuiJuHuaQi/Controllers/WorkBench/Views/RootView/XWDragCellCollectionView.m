...
@property (nonatomic, strong) UIPanGestureRecognizer *longPressGesture;
...
@property (nonatomic, strong)NSArray *markSource;
@end
...
- (instancetype)initWithFrame:(CGRect)frame collectionViewLayout:(nonnull UICollectionViewLayout *)layout withType:(NSInteger)type
{
    _type = type;
...
- (void)xwp_initializeProperty{
    ...
    if (self.type == 0) {
        ...
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(collectionViewaddCell:) name:addCell object:nil];
    }
 ...
 */
- (void)xwp_addGesture{
    UIPanGestureRecognizer *longPress = [[UIPanGestureRecognizer alloc] initWithTarget:self action:@selector(xwp_longPressed:)];
...
-(void)setGestureOpen:(BOOL)gestureOpen{
    _gestureOpen = gestureOpen;
 ...
 *  监听手势的改变
 */
- (void)xwp_longPressed:(UIPanGestureRecognizer *)longPressGesture{
 ...
 *  手势开始
 */
- (void)xwp_gestureBegan:(UIPanGestureRecognizer *)longPressGesture{
    
    //获取手指所在的cell
    _originalIndexPath = [self indexPathForItemAtPoint:[longPressGesture locationOfTouch:0 inView:longPressGesture.view]];
    HQBenchCollectionCell *cell = (HQBenchCollectionCell *)[self cellForItemAtIndexPath:_originalIndexPath];
    
    if (self.type < 2) {
        
        HQRootModel *rootModel = cell.rootModel;
        if (rootModel.functionModelType == FunctionModelTypeMore) {
            return;
        }
        cell.minusButton.hidden = NO;
        cell.layer.cornerRadius = 5;
        cell.layer.masksToBounds = YES;
        cell.backgroundColor = HexAColor(0x000000, 0.05);
    }else if (self.type == 2){
 ...
 *  手势拖动
 */
- (void)xwp_gestureChange:(UIPanGestureRecognizer *)longPressGesture{
    //通知代理
 ...
 *  手势取消或者结束
 */
- (void)xwp_gestureEndOrCancle:(UIPanGestureRecognizer *)longPressGesture{
    HQBenchCollectionCell *cell = (HQBenchCollectionCell *)[self cellForItemAtIndexPath:_originalIndexPath];
...
/** 增加Cell */
- (void)collectionViewaddCell:(NSNotification*)noti{
    HQLog(@"%@ --- %@", noti.userInfo, noti.object);
    
    HQBenchCollectionCell *cell = (HQBenchCollectionCell *)noti.object;
    ...
    if (indexPath) {
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:@"BenchCellAddItem" object:cell.rootModel userInfo:nil];
...
//    [orignalSection removeObject:orignalSection[indexPath.item]]; //这种删除方式当数据有相同时，会出现bug
    ...
    if ([self.delegate respondsToSelector:@selector(deleteCollectionViewCellToFreshTableView)]) {
        [self.delegate deleteCollectionViewCellToFreshTableView];
    }
}
...
- (void)xwp_moveCell{
    for (UICollectionViewCell *cell in [self visibleCells]) {
        ...
        if (spacingX <= _tempMoveCell.bounds.size.width / 2.0f && spacingY <= _tempMoveCell.bounds.size.height / 2.0f) {
            ...
            HQBenchCollectionCell *cell = (HQBenchCollectionCell *)[self cellForItemAtIndexPath:_moveIndexPath];
            ...
            if (cell.type == 0) {
                ...
                NSArray *arr = temp[0];
                // 遇到最后一个不移动位置
                if (_moveIndexPath.row == arr.count - 1) {
                    continue;
                }
...
//    anim.removedOnCompletion = NO;
    ...
    for (HQBenchCollectionCell *cell in cells) {
