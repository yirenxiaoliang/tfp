...
@interface TFCustomAttributeTextCell ()<WKUIDelegate,WKNavigationDelegate,UIScrollViewDelegate,WKScriptMessageHandler>
...
@implementation TFCustomAttributeTextCell
...
//        }];
        ...
        placehoder.textColor = ExtraLightBlackTextColor;
...
-(void)setModel:(TFCustomerRowsModel *)model{
    ...
    if (IsStrEmpty(self.model.fieldValue)) {
        self.placehoder.text = @"未填写";
    }else{
...
//        }];
        ...
        if ([self.model.field.structure isEqualToString:@"1"]) {
            
            if (self.webViewHeight != size.height) {
                
                self.webViewHeight = size.height;
                self.model.height = @((self.webViewHeight) < 150 ? 150 : (self.webViewHeight));
...
- (void)userContentController:(WKUserContentController *)userContentController
    ...
    if ([message.name isEqualToString:@"linkPostMessage"]) {// 点击超链接
...
+ (instancetype)customAttributeTextCellWithTableView:(UITableView *)tableView type:(NSInteger)type index:(NSInteger)index{
    ...
    NSString *indentifier = [NSString stringWithFormat:@"TFCustomAttributeTextCell%ld",index];
    TFCustomAttributeTextCell *cell = [tableView dequeueReusableCellWithIdentifier:indentifier];
    if (!cell) {
        cell = [[TFCustomAttributeTextCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:indentifier type:type];
    }
