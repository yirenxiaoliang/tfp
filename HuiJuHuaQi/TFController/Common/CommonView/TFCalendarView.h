...
@protocol TFCalendarViewDelegate <NSObject>
- (void)calendarViewConfirmClickWithStartDate:(NSInteger)startDate endDate:(NSInteger)endDate;
@end

@interface TFCalendarView : UIView

@property (nonatomic,weak)id<TFCalendarViewDelegate> delegate;
@property (nonatomic,assign)long long startDate;// 选中开始时间
@property (nonatomic,assign)long long endDate;// 选中结束时间
