...
#define kDefaultCalendarBasicColor [UIColor whiteColor]
...
- (instancetype)initWithFrameOrigin:(CGPoint)origin width:(CGFloat)width {
    ...
    if (self = [super initWithFrame:CGRectMake(origin.x, origin.y, width, _calendarHeight)]) {
        ...
        [self setupArrowWithHeight:calendarHeaderHeight];
        // 注册 Notification 监听
...
-(void)setSelectDate:(NSDate *)selectDate{
    ...
    self.calendarScrollView.selectDate = selectDate;
    [self refreshToCurrentMonthAction:self.calendarHeaderButton];
}
...
- (void)setupArrowWithHeight:(CGFloat)height{
    
    UIButton *button = [UIButton buttonWithType:UIButtonTypeCustom];
    button.frame = CGRectMake(self.width/2 - 100, 0, height, height);
    button.backgroundColor = self.calendarBasicColor;
    [button setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
    button.titleLabel.font = [UIFont systemFontOfSize:16.0];
    ...
    button1.frame = CGRectMake(self.width/2 + 100-height, 0, height, height);
    button1.backgroundColor = self.calendarBasicColor;
    [button1 setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
    button1.titleLabel.font = [UIFont systemFontOfSize:16.0];
    [button1 addTarget:self action:@selector(rightClicked) forControlEvents:UIControlEventTouchUpInside];
    [button1 setTitleColor:LightBlackTextColor forState:UIControlStateNormal];
...
- (void)leftClicked{
    ...
    [self.calendarScrollView leftScroll];
}

- (void)rightClicked{
    
    [self.calendarScrollView rightScroll];
}
...
- (GFCalendarScrollView *)setupCalendarScrollViewWithFrame:(CGRect)frame {
    ...
    scrollView.calendarBasicColor = GreenColor;
    return scrollView;
}
...
-(void)setSelectDateAction:(ActionParameter)selectDateAction{
    _selectDateAction = selectDateAction;
    
    if (_calendarScrollView != nil) {
        _calendarScrollView.selcteDateAction = _selectDateAction;
    }
...
- (void)refreshToCurrentMonthAction:(UIButton *)sender {
    ...
    NSInteger year = [self.selectDate dateYear];
    NSInteger month = [self.selectDate dateMonth];
