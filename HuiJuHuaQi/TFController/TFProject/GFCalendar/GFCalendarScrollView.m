...
//            if ((monthInfo.month == [[NSDate date] dateMonth]) && (monthInfo.year == [[NSDate date] dateYear])) {
            if ((monthInfo.month == [_selectDate dateMonth]) && (monthInfo.year == [_selectDate dateYear])) {
                
//                if (indexPath.row == [[NSDate date] dateDay] + firstWeekday - 1) {
                if (indexPath.row == [_selectDate dateDay] + firstWeekday - 1) {
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (self.didSelectDayHandler != nil || self.selcteDateAction != nil) {
        ...
        long long inter = [HQHelper changeTimeToTimeSp:[NSString stringWithFormat:@"%ld-%02ld-%ld 12:00",year,month,day] formatStr:@"yyyy-MM-dd HH:mm"];
        self.selectDate = [NSDate dateWithTimeIntervalSince1970:inter/1000];
        cell.todayCircle.backgroundColor = _calendarBasicColor;
        if (self.didSelectDayHandler) {
            self.didSelectDayHandler(year, month, day); // 执行回调
        }
        if (self.selcteDateAction) {
            self.selcteDateAction(self.selectDate);
        }
    }
...
- (void)leftScroll{
    ...
    [self setContentOffset:CGPointMake(self.bounds.size.width, 0.0) animated:YES]; // 然后变换位置
    [_collectionViewL reloadData]; // 最后两边的 collectionView 也刷新数据
    [_collectionViewR reloadData];
