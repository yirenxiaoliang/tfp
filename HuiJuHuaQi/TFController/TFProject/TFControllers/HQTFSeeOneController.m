...
@interface HQTFSeeOneController ()<UICollectionViewDelegate,UICollectionViewDataSource,FDActionSheetDelegate,HQTFProjectCollectionViewCellDelegate,HQTFProjectCollectionViewDelegate>
...
@implementation HQTFSeeOneController
...
- (void)setupCollectionView{
    ...
    self.collectionView = [[HQTFProjectCollectionView alloc] initWithFrame:CGRectMake(0, 64, SCREEN_WIDTH, SCREEN_HEIGHT-64-55) collectionViewLayout:flowLayout];
...
-(void)dragLeftOrRightWithProjectCollectionViewCell:(HQTFProjectCollectionViewCell *)projectCollectionViewCell withMoveCell:(UIView *)moveCell withDirection:(XWDragCellCollectionViewScrollDirection)direction moveCellCenter:(CGPoint)point withModel:(id)model{
    ...
    NSInteger num = (NSInteger)self.collectionView.contentOffset.x/SCREEN_WIDTH;
        ...
        case XWDragCellCollectionViewScrollDirectionLeft:
        {
            if (num <= 0) {
                ...
                if (self.moveView == nil) {
                    ...
                    CGRect rect = [projectCollectionViewCell.tableView convertRect:moveCell.frame toView:self.collectionView];
                    self.moveView = [moveCell snapshotViewAfterScreenUpdates:NO];
                    self.moveView.frame = CGRectMake(rect.origin.x+15, rect.origin.y+15+12, rect.size.width-30, rect.size.height-30-24);
                    [self.collectionView addSubview:self.moveView];
                }
                
                [self.collectionView setContentOffset:CGPointMake(self.collectionView.contentOffset.x - SCREEN_WIDTH, self.collectionView.contentOffset.y) animated:YES];
            }
        }
            break;
        case XWDragCellCollectionViewScrollDirectionRight:
        {
            if (num >= self.seeBoards.count-1) {
                return;
            }else{
                ...
                [UIView animateWithDuration:0.25 animations:^{
                    ...
                    self.collectionView.contentOffset = CGPointMake(self.collectionView.contentOffset.x + SCREEN_WIDTH, self.collectionView.contentOffset.y);
                    self.moveView.x += SCREEN_WIDTH;
                }];
