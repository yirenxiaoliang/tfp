...
@interface TFAllDataView ()<UITableViewDelegate,UITableViewDataSource>
...
-(void)refreshAllDataViewWithDatas:(NSArray *)datas{
    ...
    if ([self.delegate respondsToSelector:@selector(allDataView:didChangeHeight:)]) {
        if (self.dynamics.count == 0) {
            [self.delegate allDataView:self didChangeHeight:SCREEN_WIDTH];
        }else{
            [self.delegate allDataView:self didChangeHeight:60*self.dynamics.count < SCREEN_WIDTH ? SCREEN_WIDTH :60*self.dynamics.count];
        }
    }
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    [cell refreshTaskDetailCommCellWithModel:self.dynamics[indexPath.row]];
    return cell;
}
