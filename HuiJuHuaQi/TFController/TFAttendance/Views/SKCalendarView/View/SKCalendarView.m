...
- (void)customView
{
    ...
    self.monthBackgroundLabel.hidden = YES;
}
...
- (NSString *)getHolidayAndSolarTermsWithChineseDay:(NSString *)chineseDay
{
    ...
    NSString * date = getNoneNil(self.calendarManage.chineseCalendarDay[row]);
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (collectionView == self.calendarCollectionView) {
        ...
        if (self.dataList) {
            if ((self.calendarManage.dayInWeek-1 <= indexPath.row) && (indexPath.row-self.calendarManage.dayInWeek+1 < self.dataList.count)) {
                TFCalendarMonthItemModel *model = self.dataList[indexPath.row-self.calendarManage.dayInWeek+1];
                cell.state = [model.state integerValue];
            }else{
                cell.state = -1;
            }
        }
...
//            cell.dateColor = [UIColor colorWithRed:204 / 255.0 green:228 / 255.0 blue:236 / 255.0 alpha:1.0];
        ...
        if (self.theDayInMonth == indexPath.row && self.calendarManage.month == self.calendarManage.theMonth && self.theYear == self.calendarManage.year) {
            ...
            cell.calendarTitle = getNoneNil(self.calendarManage.chineseCalendarDay[indexPath.row]);// 农历日期
