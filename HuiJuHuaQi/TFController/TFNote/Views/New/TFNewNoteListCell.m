...
#import "TFNewNoteListCell.h"

@interface TFNewNoteListCell ()
...
@implementation TFNewNoteListCell
...
+ (instancetype)TFNewNoteListCell {
    
    return [[[NSBundle mainBundle] loadNibNamed:@"TFNewNoteListCell" owner:self options:nil] lastObject];
}

+ (instancetype)NewNoteListCellWithTableView:(UITableView *)tableView {
    
    static NSString *ID = @"TFNewNoteListCell";
    TFNewNoteListCell *cell = [tableView dequeueReusableCellWithIdentifier:ID];
    if (!cell) {
        cell = [self TFNewNoteListCell];
    }
