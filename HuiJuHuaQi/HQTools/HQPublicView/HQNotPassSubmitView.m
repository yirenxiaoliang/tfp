...
@interface HQNotPassSubmitView () <UITextViewDelegate>
...
@implementation HQNotPassSubmitView
...
- (void)awakeFromNib
{
    ...
    self.frame = CGRectMake(30, (SCREEN_HEIGHT - 255)/2, SCREEN_WIDTH - 60, 175);
...
- (void)keyboardShow:(NSNotification *)niti{
    ...
    [UIView animateWithDuration:0.25 animations:^{
        self.frame = CGRectMake(30, frame.origin.y - 255 - 20, SCREEN_WIDTH - 60, self.height);
    }];
}
...
- (IBAction)sureAction:(id)sender
{
    ...
    if (IsStrEmpty(self.contentTextView.text)) {
        [MBProgressHUD showError:@"请输入" toView:KeyWindow];
        return ;
    }
...
+ (void)submitPlaceholderStr:(NSString *)placeholderStr
    ...
    // 告警窗体
    HQNotPassSubmitView *view = [[[NSBundle mainBundle] loadNibNamed:@"HQNotPassSubmitView" owner:nil options:nil] firstObject];
    ...
    if (IsStrEmpty(title)) {
        view.topW.constant = 15;
        view.contentTextView.keyboardType = UIKeyboardTypeDefault;
    }else{
        view.height = 230;
    ...
    view.title.textAlignment = NSTextAlignmentLeft;
    view.contentTextView.text = content;
    view.onDismiss = ^(void){
        [UIView animateWithDuration:0.25 animations:^{
            bgView.alpha = 0;
        }completion:^(BOOL finished) {
