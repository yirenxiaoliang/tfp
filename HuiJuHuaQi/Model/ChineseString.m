...
#pragma mark - 返回联系人Model
+(NSMutableArray*)LetterSortModelArray:(NSArray*)modelArr
{
    
    NSMutableArray *nameMutArr = [[NSMutableArray alloc] init];
    for (TFEmployeeCModel * employModel in modelArr) {
        
        [nameMutArr addObject:employModel.employee_name];
    }
    
    NSMutableArray *tempArray = [self ReturnSortChineseArrar:nameMutArr];
    NSMutableArray *LetterResult = [NSMutableArray array];
    NSMutableArray *item = [NSMutableArray array];
    NSString *tempString;
    
    
    if (modelArr.count != nameMutArr.count ||  nameMutArr.count != tempArray.count) {
    ...
    for (int i=0; i<tempArray.count; i++) {
        ...
        NSString *object = [tempArray[i] nullProcess];
...
+(NSMutableArray*)ReturnSortChineseArrar:(NSArray*)stringArr
{
    ...
    for(int i=0;i<[stringArr count];i++)
    {
        ...
        // HQLog(@"string====%@",chineseString.string);
        ...
        {
            HQLog(@"chineseString.string== %@",chineseString.string);
            //首字母大写
            ...
            if(![chineseString.string isEqualToString:@""]){
                ...
                for(int j=0;j<chineseString.string.length;j++){
                    ...
                    //                    HQLog(@"singlePinyinLetter ==%@",singlePinyinLetter);
...
//        HQLog(@"chineseStringsArray====%@",((ChineseString*)[chineseStringsArray objectAtIndex:i]).pinYin);
//    }
    HQLog(@"-----------------------------");
...
+(NSMutableArray*)SortArray:(NSArray*)stringArr
{
    ...
    for(int i=0;i<[stringArr count];i++){
        ...
        HQLog(@"SortArray----->%@",((ChineseString*)[tempArray objectAtIndex:i]).string);
    }
    return result;
}
