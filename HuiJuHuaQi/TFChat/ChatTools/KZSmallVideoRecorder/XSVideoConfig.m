...
void xs_dispatch_after(float time, dispatch_block_t block)
{
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(time * NSEC_PER_SEC)), dispatch_get_main_queue(), block);
}

@implementation XSVideoConfig

+ (CGRect)viewFrameWithType:(XSVideoViewShowType)type {
    if (type == XSVideoViewShowTypeSingle) {
        return [UIScreen mainScreen].bounds;
    }
    CGFloat viewHeight = xsSCREEN_WIDTH/xsVideo_w_h + 20 + xsControViewHeight;
    return CGRectMake(0, xsSCREEN_HEIGHT - viewHeight, xsSCREEN_WIDTH, viewHeight);
}

+ (CGSize)videoViewDefaultSize {
    return CGSizeMake(xsSCREEN_WIDTH, xsSCREEN_WIDTH/xsVideo_w_h);
}

+ (CGSize)defualtVideoSize {
    return CGSizeMake(xsVideoWidthPX, xsVideoWidthPX/xsVideo_w_h);
}
...
+ (NSArray *)getSortVideoList {
    ...
    NSArray *sortList = [oldList sortedArrayUsingComparator:^NSComparisonResult(id  _Nonnull obj1, id  _Nonnull obj2) {
        XSVideoModel *model1 = obj1;
        XSVideoModel *model2 = obj2;
...
+ (NSString *)getVideoPath {
    return [self getDocumentSubPath:xsVideoDicName];
}
